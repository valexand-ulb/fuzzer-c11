...
void attempt2(char* cmd) {
    ...
    initialize_tar_headers(&header1, filenames[0], 5, time(NULL));
    // -------- header tweak --------
    strncpy(header->name, "\xff", sizeof(header->name)-1);
    // ------------------------------
    write_tar_header(tar_ptr, &header1);
    write_tar_content(tar_ptr, "\x41\x42\x43\x44\x0a", true);
...
void attempt3(char* cmd) {
    ...
    initialize_tar_headers(&header1, filenames[0], 5, time(NULL));
    // -------- header tweak --------
    snprintf(header->size, sizeof(header->size), "%011o", 99999999999); // exactly 11, to fit size in header plus nullbyte
    // ------------------------------
    write_tar_header(tar_ptr, &header1);
    write_tar_content(tar_ptr, "\x41\x42\x43\x44\x0a", true);
...
void attempt4(char* cmd) {
    ...
    initialize_tar_headers(&header1, filenames[0], 5, time(NULL));
    // -------- header tweak --------
    snprintf(header->size, sizeof(header->size), "%011o", 999999999999); // exactly 12, to overflow the nullbyte
    // ------------------------------
    write_tar_header(tar_ptr, &header1);
    write_tar_content(tar_ptr, "\x41\x42\x43\x44\x0a", true);
