...
void test_filename(struct tar_t *headers){
    ...
    initialize_fuzzed_tar_headers(headers, NAME_PADDING, "file1.txt", "%s");
}

void test_mode(struct tar_t *headers){
    printf("\tTesting mode...\n");
    initialize_fuzzed_tar_headers(headers, MODE_PADDING, "0644", "%07o");
}

void test_uid(struct tar_t *headers){
    printf("\tTesting uid...\n");
    initialize_fuzzed_tar_headers(headers, UID_PADDING, "0000000", "%07o");
}

void test_gid(struct tar_t *headers){
    printf("\tTesting gid...\n");
    initialize_fuzzed_tar_headers(headers, GID_PADDING, "0000000", "%07o");
}

void test_size(struct tar_t *headers){
    printf("\tTesting size...\n");
    initialize_fuzzed_tar_headers(headers, SIZE_PADDING, "00000000000", "%011o");
}

void test_mtime(struct tar_t *headers){
    printf("\tTesting mtime...\n");
    initialize_fuzzed_tar_headers(headers, MTIME_PADDING, "00000000000", "%011o");
}
...
void test_checksum(struct tar_t *headers){
    ...
    initialize_fuzzed_tar_headers(headers, CHKSUM_PADDING, headers->chksum, "%07o");
}
